...
const AuthLoader: React.FC<AuthLoaderProps> = ({ 
  ...
  useEffect(() => {
    ...
    setDisplayMessage(message || defaultMessage);

    // Timer para cambiar el mensaje después de 5 segundos
    const timer = setTimeout(() => {
      setIsLongWait(true);
      setDisplayMessage(longWaitMessage);
    }, 5000);

    // Limpiar el timer cuando el componente se desmonte
    return () => clearTimeout(timer);
  }, [message, isLogin, defaultMessage, longWaitMessage]);
    ...
    <div className="flex flex-col items-center justify-center py-12 sm:py-16 px-4">
      ...
      <div className="relative mb-8">
        ...
        <div className="relative w-20 h-20 sm:w-24 sm:h-24 flex items-center justify-center bg-gradient-to-br from-orange-400 to-orange-600 dark:from-orange-500 dark:to-orange-700 rounded-full shadow-xl">
          {isLogin ? (
            <LogIn className="w-10 h-10 sm:w-12 sm:h-12 text-white animate-bounce" style={{ animationDuration: "1.5s" }} />
          ) : (
            <Lock className="w-10 h-10 sm:w-12 sm:h-12 text-white animate-bounce" style={{ animationDuration: "1.5s" }} />
          )}
        </div>
      </div>
      ...
      <div className="text-center mb-6">
        <h3 
          className={`text-lg sm:text-xl font-semibold text-gray-900 dark:text-gray-100 mb-3 ${
            isLongWait ? "animate-fade-in" : ""
          }`}
          key={displayMessage}
        >
          {displayMessage}
