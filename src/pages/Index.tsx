...
const CourseCard = ({
  ...
  const imageSrc =
    course.imagen === "" || imageError ? "/placeholder.svg" : course.imagen;
  ...
  return (
    ...
    >
      <CardContent className="p-0 flex flex-row sm:flex-row h-auto sm:min-h-[200px]">
        {/* Imagen del curso */}
        <div className="w-24 h-24 sm:w-48 sm:h-auto sm:self-stretch relative overflow-hidden flex-shrink-0 bg-slate-100 dark:bg-slate-800">
          {imageLoading && (
...
const Index = () => {
  ...
  useEffect(() => {
    ...
    fetchCourses();
  }, [user?.uid, user?.activo, enabledModules]);

  // Progreso calculado localmente sin llamadas extra al backend
  const courseProgressMap = useMemo<Record<string, number>>(() => {
    const map: Record<string, number> = {};
    for (const { course, modules } of enrichedCourses) {
      const enabledMods = modules.filter((m) => m && enabledModules[m.id] !== false);
      let completed = 0;
      let total = 0;
      for (const modulo of enabledMods) {
        ...
        if (modulo.url_video) {
          ...
          videos.forEach((_: any, index: number) => {
            total++;
            if (moduleProgress[`${modulo.id}_video_${index}`] === true) completed++;
          });
        }
        if (modulo.url_archivo) {
          const docs = modulo.url_archivo.includes("|||")
          ...
          docs.forEach((_: any, index: number) => {
            total++;
            if (moduleProgress[`${modulo.id}_document_${index}`] === true) completed++;
          });
        }
      }
                  ...
                  <div className="flex flex-col items-center justify-center text-center space-y-6">
                    ...
                    <div className="space-y-3">
                      ...
                      <p className="text-sm sm:text-base text-red-600 dark:text-red-400 max-w-2xl">
                        Por favor, contacta con el administrador para revertir
                        esta acción y recuperar el acceso a tus formaciones.
                      </p>
                    </div>
